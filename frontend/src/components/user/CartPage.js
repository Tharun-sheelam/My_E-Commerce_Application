  ...
  useEffect(() => {
    ...
    const fetchUserCart = async () => {
      ...
      try {
        ...
        const apiUrl2 = 'http://localhost:8080/shoppinghub/user/cart/fetch/'+userId;
        ...
        const apiUrl = 'http://localhost:8080/shoppinghub/user/getprofiledetails/'+userEmail;
    ...
    <>
      ...
      <div className="cart-container">
        ...
        {orders.map((order, index) => (
          ...
          <p>
  Order Date:{" "}
  {new Date(order.dateTime).toLocaleDateString("hi-IN")}{" "}
  <br></br>order Time:{" "}
  {new Date(order.dateTime).toLocaleTimeString("hi-IN")}
</p>
          </div>
        ))}
      </div>
